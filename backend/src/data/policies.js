export const policies = [
  {
    id: "KB-01",
    title: "Password Reset",
    content:
      "Employees can reset their own password via the self-service portal at any time. If locked out after 5 failed attempts, contact IT to unlock the account manually. No approval required.",
    category: "password",
  },

  {
    id: "KB-02",
    title: "VPN Access",
    content:
      "VPN access is granted automatically to all full-time employees. Contractors require manager approval submitted via the access request form. VPN credentials expire every 90 days and must be renewed by the employee.",
    category: "vpn",
  },

  {
    id: "KB-03",
    title: "Laptop Replacement",
    content:
      "Laptops are eligible for replacement after 3 years of service, or earlier in case of verified hardware failure. Requests must be raised at least 2 weeks in advance of intended replacement.",
    category: "laptop",
  },

  {
    id: "KB-04",
    title: "Software Installation Requests",
    content:
      "Standard software listed in the approved catalog can be self-installed. Non-catalog software requires IT Security review, which takes 3–5 business days.",
    category: "software",
  },

  {
    id: "KB-05",
    title: "Printer Troubleshooting",
    content:
      "For printer issues, first check the printer queue and restart the print spooler. If the issue persists after restart, log a ticket with the printer asset tag.",
    category: "printer",
  },

  {
    id: "KB-06",
    title: "Email Mailbox Quota",
    content:
      "Default mailbox quota is 25GB. Employees nearing quota should archive old mail. Quota increases beyond 25GB require manager approval and are capped at 50GB.",
    category: "email",
  },

  {
    id: "KB-07",
    title: "Guest Wi-Fi Access",
    content:
      "Guest Wi-Fi credentials are valid for 24 hours and can be generated by any employee from the front-desk kiosk. No IT ticket required.",
    category: "wifi",
  },

  {
    id: "KB-08",
    title: "Expense Software Access",
    content:
      "Access to the expense management tool is granted by Finance, not IT. IT can only assist with login or technical issues once an account already exists.",
    category: "expense",
  },

  {
    id: "KB-09",
    title: "Security Incident Reporting",
    content:
      "Any suspected phishing email, malware, or unauthorized access attempt must be reported immediately to security@veridian-corp.example and should not be forwarded to other employees.",
    category: "security",
  },

  {
    id: "KB-10",
    title: "Work From Home Equipment",
    content:
      "Employees working remotely more than 3 days per week are eligible for a one-time home office equipment allowance. Requires manager sign-off and Finance processing. IT only handles shipping once approved.",
    category: "equipment",
  },

  {
    id: "ASSET-01",
    title: "Asset Management Policy",
    content:
      "All company-issued hardware follows a standard 4-year refresh cycle from date of issue. Early replacement outside this cycle requires Finance sign-off in addition to IT approval.",
    category: "asset",
  },
];